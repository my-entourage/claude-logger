...
for i in {1..5}; do
  cat > "$TEST_TMPDIR/.claude/sessions/orphan-$i.json" << EOF
{
  "session_id": "orphan-$i",
  "status": "in_progress",
...
EOF
  ...
  touch "$TEST_TMPDIR/.claude/sessions/orphan-$i.json"
done
...
orphan_count=0
for i in {1..5}; do
  status=$(jq -r '.status' "$TEST_TMPDIR/.claude/sessions/orphan-$i.json")
...
cat > "$TEST_TMPDIR/.claude/sessions/already-incomplete.json" << 'EOF'
{
...
EOF
touch "$TEST_TMPDIR/.claude/sessions/already-incomplete.json"
...
status=$(jq -r '.status' "$TEST_TMPDIR/.claude/sessions/already-incomplete.json")
...
echo "not json" > "$TEST_TMPDIR/.claude/sessions/corrupted-orphan.json"
touch "$TEST_TMPDIR/.claude/sessions/corrupted-orphan.json"
...
content=$(cat "$TEST_TMPDIR/.claude/sessions/corrupted-orphan.json")
...
cat > "$TEST_TMPDIR/.claude/sessions/complete-session.json" << 'EOF'
{
...
EOF
touch "$TEST_TMPDIR/.claude/sessions/complete-session.json"

original=$(cat "$TEST_TMPDIR/.claude/sessions/complete-session.json")

# Start new session
input='{"session_id":"new-session4","cwd":"'"$TEST_TMPDIR"'","source":"startup"}'
run_hook "session_start.sh" "$input"

current=$(cat "$TEST_TMPDIR/.claude/sessions/complete-session.json")
...
cat > "$TEST_TMPDIR/.claude/sessions/no-status.json" << 'EOF'
{
  "session_id": "no-status",
  "start": {"timestamp": "2025-01-01T12:00:00Z"}
}
EOF
touch "$TEST_TMPDIR/.claude/sessions/no-status.json"
...
echo "99999" > "$TEST_TMPDIR/.claude/sessions/.lock"
...
if [ $elapsed -lt 10 ]; then
  if [ -f "$TEST_TMPDIR/.claude/sessions/test-lock.json" ]; then
...
if [ ! -f "$TEST_TMPDIR/.claude/sessions/.lock" ]; then
...
created=$(ls -1 "$TEST_TMPDIR/.claude/sessions/"*.json 2>/dev/null | grep -c "rapid-" || echo 0)
...
# Count created sessions
created=$(ls -1 "$TEST_TMPDIR/.claude/sessions/"*.json 2>/dev/null | grep -c "parallel-" || echo 0)
...
cat > "$TEST_TMPDIR/.claude/sessions/overlap-test.json" << 'EOF'
{
...
if [ -f "$TEST_TMPDIR/.claude/sessions/new-overlap.json" ]; then
  end_status=$(jq -r '.status' "$TEST_TMPDIR/.claude/sessions/overlap-test.json")
...
# Create a session
cat > "$TEST_TMPDIR/.claude/sessions/double-end.json" << 'EOF'
{
...
status=$(jq -r '.status' "$TEST_TMPDIR/.claude/sessions/double-end.json")
